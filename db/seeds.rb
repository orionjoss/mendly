...
recommendations = Recommendation.all
  ...
  message = Message.create(
    content: "Just discovered 'Serial.' True crime storytelling at its finest! 🎙️🕵️",
    recommendation: Recommendation.where(item: Item.find_by(title: "Serial")).sample,
    user: User.all.sample,
    chatroom: Chatroom.find_by(name: "AI Library Assistant")
  )

  message = Message.create(
    content: "Discovering the magic of Pink Floyd. 'Comfortably Numb' is a masterpiece! 🎸🌌",
    recommendation: Recommendation.where(item: Item.find_by(title: "Pink Floyd")).sample,
    user: User.all.sample,
    chatroom: Chatroom.find_by(name: "AI Library Assistant")
  )

  message = Message.create(
  content: "Check out 'Vsauce' for mind-bending science and brain teasers! 🧠🔍",
  recommendation: Recommendation.where(item: Item.find_by(title: "Vsauce")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "Gaming paradise: 'The Legend of Zelda: Breath of the Wild.' Open-world adventure! 🎮🗡️",
  recommendation: Recommendation.where(item: Item.find_by(title: "The Legend of Zelda: Breath of the Wild")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "Just saw 'Inception.' Mind-bending plot, stunning visuals, and Leonardo DiCaprio at his best! 🎬🔥",
  recommendation: Recommendation.where(item: Item.find_by(title: "Inception")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "If you're into action, 'John Wick' is the way to go. Keanu Reeves is a total badass! 💥🔫",
  recommendation: Recommendation.where(item: Item.find_by(title: "John Wick")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "Watched 'The Shawshank Redemption.' It's a classic for a reason. Epic storytelling! 🍿📺",
  recommendation: Recommendation.where(item: Item.find_by(title: "The Shawshank Redemption")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "I laughed so hard watching 'Superbad.' Hilarious coming-of-age comedy! 😂🎉",
  recommendation: Recommendation.where(item: Item.find_by(title: "Superbad")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "'Black Panther' is a masterpiece! Wakanda forever! 🙅🏿‍♂️🐾",
  recommendation: Recommendation.where(item: Item.find_by(title: "Black Panther")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)

message = Message.create(
  content: "Sci-fi fan? Check out 'Blade Runner 2049.' Visually stunning! 🤖🌆",
  recommendation: Recommendation.where(item: Item.find_by(title: "Blade Runner 2049")).sample,
  user: User.all.sample,
  chatroom: Chatroom.find_by(name: "AI Library Assistant")
)
